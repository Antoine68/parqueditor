...
public class MainController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		createListeners();
		contentTable.setRowFactory(new Callback<TableView<Map<FieldAdapter,StringProperty>>, TableRow<Map<FieldAdapter,StringProperty>>>() {
			@Override
			public TableRow<Map<FieldAdapter, StringProperty>> call(TableView<Map<FieldAdapter, StringProperty>> param) {
	...
	private void addFieldControl(FieldAdapter field) {
		...
		col.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<Map<FieldAdapter, StringProperty>,String>, ObservableValue<String>>() {
			@Override
			public ObservableValue<String> call(CellDataFeatures<Map<FieldAdapter, StringProperty>, String> param) {
				return param.getValue().computeIfAbsent(field, f -> {
					StringProperty s = new SimpleStringProperty();
					s.addListener((obs, old, val) -> {
						if (field.getDefaultValue() != null && !field.getDefaultValue().isBlank() && (val == null || val.isBlank())) {
	...
	private <T> void removeFieldControlFromList(List<T> nodes, FieldAdapter field) {
	    ...
	    while (i < nodes.size() && idx < 0) {
	        ...
	        if (node instanceof Node && field.equals((FieldAdapter)((Node) node).getUserData())) {
	            idx = i;
	        } else if (node instanceof TableColumn && field.equals((FieldAdapter)((TableColumn<?, ?>) node).getUserData())) {
	            idx = i;
	        }
	        i++;
	    }
